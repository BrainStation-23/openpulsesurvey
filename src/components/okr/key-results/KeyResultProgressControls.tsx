...
export const KeyResultProgressControls: React.FC<KeyResultProgressControlsProps> = ({
  ...
  if (keyResult.measurementType === 'boolean') {
    return <div className="flex items-center justify-between mb-4 mt-2">
        <span className="text-sm font-medium">Completed</span>
        <Switch checked={keyResult.booleanValue} onCheckedChange={onBooleanChange} disabled={isPending} />
      </div>;
  }
  let unit = '';
  ...
  return <div className="space-y-4 mb-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          <Input type="number" value={progressValue} onChange={handleInputChange} min={keyResult.startValue} max={keyResult.targetValue} step={keyResult.measurementType === 'percentage' ? 5 : 1} disabled={isPending} className="w-full text-right" />
          <span className="text-sm w-4">{unit}</span>
          <Button variant="outline" size="sm" onClick={handleUpdate} disabled={isPending || progressValue === keyResult.currentValue}>
            Update
          </Button>
        </div>
      </div>
      <div className="space-y-1">
        ...
        <Slider value={[progressValue]} min={keyResult.startValue} max={keyResult.targetValue} step={keyResult.measurementType === 'percentage' ? 5 : 1} onValueChange={handleSliderChange} disabled={isPending} className="mt-2" />
      </div>
    </div>;
};