...
export function ImportProgress({ progress, paused, onPauseToggle, onCancel }: ImportProgressProps) {
  ...
  const progressValue = progress ? (progress.processed / progress.total) * 100 : 0;
  const hasErrors = progress && progress.errors.length > 0;
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="flex flex-col">
            ...
            <span className="text-sm font-medium">
              {progress ? `${progress.currentBatch} of ${progress.totalBatches}` : '-'}
            </span>
          </div>
          <div className="flex flex-col">
            <span className="text-xs text-gray-500">Users processed</span>
            <span className="text-sm font-medium">
              {progress ? `${progress.processed} of ${progress.total}` : '-'}
            </span>
          </div>
          <div className="flex flex-col">
            ...
            <span className="text-sm font-medium flex items-center">
              ...
              {progress ? formatEstimatedTime(progress.estimatedTimeRemaining) : '-'}
            </span>
          </div>
          <div className="flex flex-col">
