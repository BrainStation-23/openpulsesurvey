...
export default function Presentation() {
  ...
  function MiniMap() {
    return <div className="flex gap-1 items-center px-2">
        {Array.from({
        length: totalSlides
      }).map((_, i) => <span key={i} className={`inline-block w-2 h-2 rounded-full transition-all mx-0.5 ${currentSlide === i ? "bg-primary scale-125" : "bg-gray-300"}`} style={{
  ...
  return <div className="relative min-h-screen h-screen flex flex-col bg-background" {...swipeHandlers}>
      ...
      <SlideOverviewPanel totalSlides={totalSlides} currentSlide={currentSlide} onJump={s => {
      setCurrentSlide(s);
      setShowOverview(false);
    }} slideTitles={slideTitles} visible={showOverview} onClose={() => setShowOverview(false)} />

      {/* Notes panel */}
      <NotesPanel visible={showNotes} notes={slideNotes[currentSlide] || ""} onChange={val => setSlideNotes({
      ...slideNotes,
      [currentSlide]: val
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        transition: "transform 200ms"
          ...
          <div className="w-full h-full max-w-6xl mx-auto">
            <PresentationLayout progress={(currentSlide + 1) / totalSlides * 100} isFullscreen={isFullscreen}>
              <PublicPresentationControls campaign={campaign!} onBack={handleBack} onPrevious={() => setCurrentSlide(prev => Math.max(0, prev - 1))} onNext={() => setCurrentSlide(prev => Math.min(totalSlides - 1, prev + 1))} onFullscreen={toggleFullscreen} isFirstSlide={currentSlide === 0} isLastSlide={currentSlide === totalSlides - 1} isFullscreen={isFullscreen} currentSlide={currentSlide + 1} totalSlides={totalSlides} />
