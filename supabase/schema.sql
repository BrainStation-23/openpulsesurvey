...
CREATE OR REPLACE FUNCTION "public"."activate_campaign_instances"() RETURNS integer
...
ALTER FUNCTION "public"."activate_campaign_instances"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."check_user_survey_access"("p_user_id" "uuid", "p_survey_id" "uuid") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
BEGIN
    -- Check if user has assignment for this survey
    RETURN EXISTS (
        SELECT 1 
...
ALTER FUNCTION "public"."check_user_survey_access"("p_user_id" "uuid", "p_survey_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."complete_campaign_instances"() RETURNS integer
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
DECLARE
...
ALTER FUNCTION "public"."complete_campaign_instances"() OWNER TO "postgres";
...
BEGIN
  -- Validate sort_by parameter (removed completion_rate)
  IF p_sort_by NOT IN ('period_number', 'starts_at', 'ends_at', 'status', 'created_at', 'updated_at') THEN
  ...
  -- Return paginated results with dynamic sorting (removed completion_rate)
  RETURN QUERY EXECUTE format(
    ...
    WHERE ci.campaign_id = $1
      ...
      AND ($6 IS NULL OR ci.status::text = $6)
    ORDER BY %I %s
    LIMIT $7
    OFFSET $8',
    v_total_count,
...
CREATE OR REPLACE FUNCTION "public"."get_campaign_sbu_performance"("p_campaign_id" "uuid", "p_instance_id" "uuid") RETURNS TABLE("rank" integer, "sbu_name" "text", "total_assigned" integer, "total_completed" integer, "avg_score" double precision, "completion_rate" double precision)
  ...
  assignments AS (
    SELECT 
      sa.id AS assignment_id,
      sa.user_id,
      -- Use the denormalized primary_sbu_id from survey_responses if available, otherwise fallback to user_sbus
      COALESCE(
        (SELECT sr.primary_sbu_id FROM survey_responses sr WHERE sr.assignment_id = sa.id AND sr.campaign_instance_id = p_instance_id LIMIT 1),
        us.sbu_id
      ) AS sbu_id,
      s.name AS sbu_name,
  ...
  ratings AS (
    SELECT 
      COALESCE(sr.primary_sbu_id, us.sbu_id) AS sbu_id,
    ...
    LEFT JOIN user_sbus us ON us.user_id = sa_inner.user_id AND us.is_primary = true
    ...
    GROUP BY COALESCE(sr.primary_sbu_id, us.sbu_id)
  ),
...
CREATE OR REPLACE FUNCTION "public"."get_dimension_bool"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_question_name" "text", "p_dimension" "text") RETURNS TABLE("dimension" "text", "yes_count" bigint, "no_count" bigint, "total_count" bigint)
  ...
  response_stats AS (
    SELECT 
      ...
      CASE 
        WHEN jsonb_typeof(sr.response_data->p_question_name) = 'boolean' AND
             (sr.response_data->p_question_name)::text = 'true' THEN true
        WHEN jsonb_typeof(sr.response_data->p_question_name) = 'string' AND
             LOWER(sr.response_data->>p_question_name) IN ('true', '1', 'yes') THEN true
        ELSE false 
  ...
  dimension_stats AS (
    SELECT
      ...
      SUM(CASE WHEN boolean_value = true THEN 1 ELSE 0 END) AS yes_responses,
      SUM(CASE WHEN boolean_value = false THEN 1 ELSE 0 END) AS no_responses
    FROM response_stats
  ...
  SELECT 
    ...
    total_responses AS total_count
...
CREATE OR REPLACE FUNCTION "public"."get_dimension_radiogroup"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_question_name" "text", "p_dimension" "text") RETURNS TABLE("dimension" "text", "choice_data" "jsonb")
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $_$
BEGIN
  RETURN QUERY
  ...
  dimension_choice_stats AS (
    SELECT
      ...
      CASE 
        WHEN jsonb_typeof(sc.choice) = 'string' THEN sc.choice #>> '{}'
        ELSE sc.choice->>'value'
      END AS choice_value,
      CASE 
        WHEN jsonb_typeof(sc.choice) = 'string' THEN sc.choice #>> '{}'
        ELSE sc.choice->>'text'
      END AS choice_text,
      COUNT(rs.selected_choice) FILTER (WHERE 
        rs.selected_choice = CASE 
          WHEN jsonb_typeof(sc.choice) = 'string' THEN sc.choice #>> '{}'
          ELSE sc.choice->>'value'
        END
      ) AS choice_count,
...
ALTER FUNCTION "public"."get_dimension_radiogroup"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_question_name" "text", "p_dimension" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_dimension_satisfaction"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_question_name" "text", "p_dimension" "text") RETURNS TABLE("dimension" "text", "unsatisfied" bigint, "neutral" bigint, "satisfied" bigint, "total" bigint, "avg_score" numeric)
  ...
  dimension_stats AS (
    SELECT
      dimension_value,
      COUNT(*) AS total_responses,
      SUM(CASE WHEN rating_value <= 2 THEN 1 ELSE 0 END) AS unsatisfied_count,
      SUM(CASE WHEN rating_value = 3 THEN 1 ELSE 0 END) AS neutral_count,
      SUM(CASE WHEN rating_value >= 4 THEN 1 ELSE 0 END) AS satisfied_count,
      ROUND(AVG(rating_value)::NUMERIC, 2) AS avg_rating_score
    FROM response_stats
    WHERE dimension_value IS NOT NULL
    GROUP BY dimension_value
...
DECLARE
  ...
  v_total_count BIGINT;
BEGIN
  -- Get campaign anonymity setting
  ...
  WHERE sa.campaign_id = p_campaign_id
    ...
    AND (
      p_search_term IS NULL
      OR LOWER(s.name) LIKE '%' || LOWER(p_search_term) || '%'
      OR LOWER(CONCAT(COALESCE(sup.first_name, ''), ' ', COALESCE(sup.last_name, ''))) LIKE '%' || LOWER(p_search_term) || '%'
    );
  ...
  ELSIF p_sort_by = 'name' THEN
    -- For anonymous campaigns, sort by SBU name, otherwise by user name
    IF v_campaign_anonymous THEN
      v_sort_sql := 's.name ' || p_sort_direction;
    ELSE
      v_sort_sql := 'CONCAT(p.first_name, '' '', p.last_name) ' || p_sort_direction;
    END IF;
  ELSE
    v_sort_sql := 'sr.submitted_at DESC';
  END IF;
  ...
  RETURN QUERY EXECUTE format('
    SELECT 
      ...
      sr.state_data,
      %L::bigint AS total_count,
      %L::boolean AS campaign_anonymous,
      s.name AS primary_sbu_name,
      CONCAT(sup.first_name, '' '', sup.last_name) AS primary_supervisor_name
    FROM survey_responses sr
    JOIN survey_assignments sa ON sa.id = sr.assignment_id
    LEFT JOIN profiles p ON p.id = sr.user_id
    LEFT JOIN sbus s ON s.id = sr.primary_sbu_id
    LEFT JOIN profiles sup ON sup.id = sr.primary_supervisor_id
    WHERE sa.campaign_id = %L
      AND sr.campaign_instance_id = %L
      AND sr.status = ''submitted''
      AND (
        %L IS NULL
        OR LOWER(s.name) LIKE ''%%'' || LOWER(%L) || ''%%''
        OR LOWER(CONCAT(COALESCE(sup.first_name, ''''), '' '', COALESCE(sup.last_name, ''''))) LIKE ''%%'' || LOWER(%L) || ''%%''
      )
    ORDER BY %s
    LIMIT %s OFFSET %s',
    v_total_count,
    v_campaign_anonymous,
    p_campaign_id,
    p_instance_id,
    p_search_term,
    p_search_term,
...
CREATE OR REPLACE FUNCTION "public"."get_supervisor_team_feedback"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_supervisor_id" "uuid", "p_question_name" "text" DEFAULT NULL::"text") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $_$
DECLARE
  v_team_members UUID[];
...
ALTER FUNCTION "public"."get_supervisor_team_feedback"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_supervisor_id" "uuid", "p_question_name" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_supervisor_team_trend"("p_campaign_id" "uuid", "p_supervisor_id" "uuid", "p_question_name" "text" DEFAULT NULL::"text") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $_$DECLARE
  v_team_members UUID[];
  v_result JSON;
...
BEGIN
  ...
  -- Get all instances for this campaign for trend data
  WITH instances AS (
    SELECT 
      ci.id,
      ci.period_number,
    ...
    FROM campaign_instances ci
    WHERE ci.campaign_id = p_campaign_id
    AND ci.status::instance_status = 'completed'
    ORDER BY ci.period_number
  ),
  questions AS (
    ...
    WHERE sc.id = p_campaign_id
    AND q->>'name' IS NOT NULL
    AND q->>'type' IN ('rating', 'boolean')  -- Only include rating and boolean questions for trends
  ),
  question_trends AS (
    -- Calculate metrics for each question across instances
    SELECT 
      q.question_name,
      q.question_title,
      q.question_type,
      ...
      i.period_number,
      (
        -- Count responses for this question in this instance first
        SELECT COUNT(*)
        FROM survey_responses sr
        JOIN survey_assignments sa ON sr.assignment_id = sa.id
        WHERE sr.campaign_instance_id = i.id
        AND sa.campaign_id = p_campaign_id
        AND sr.user_id = ANY(v_team_members)
        AND sr.status = 'submitted'
        AND sr.response_data ? q.question_name
      ) AS response_count,
      (
        -- Calculate average rating or yes percentage based on question type
        CASE 
          WHEN q.question_type = 'rating' THEN
            -- For rating questions, safely convert to numeric
            (
              SELECT AVG(
                CASE 
                  WHEN sr.response_data->>q.question_name ~ '^[0-9]+(\.[0-9]+)?$' 
                  THEN (sr.response_data->>q.question_name)::NUMERIC
                  ELSE NULL
                END
              )
              FROM survey_responses sr
          ...
          WHEN q.question_type = 'boolean' THEN
            -- For boolean questions, handle both string and actual boolean values
            (
              SELECT 
                SUM(
                  CASE 
                    -- Handle actual JSON boolean true
                    WHEN jsonb_typeof(sr.response_data->q.question_name) = 'boolean' AND
                         (sr.response_data->q.question_name)::text = 'true' THEN 1
                    -- Handle string values representing true
                    WHEN jsonb_typeof(sr.response_data->q.question_name) = 'string' AND
                         LOWER(sr.response_data->>q.question_name) IN ('true', '1', 'yes') THEN 1
                    ELSE 0 
                  END
                )::NUMERIC / NULLIF(COUNT(*), 0)::NUMERIC * 100
              FROM survey_responses sr
  ...
  filtered_trends AS (
    -- Only include trend data points where we have enough responses
    SELECT *
    FROM question_trends qt
    WHERE qt.response_count >= v_min_responses_required
  )
        ...
        SELECT json_agg(
          json_build_object(
            ...
            'trend_data', (
              SELECT json_agg(
                json_build_object(
                  'instance_id', ft.instance_id,
                  'period_number', ft.period_number,
                  'avg_value', ft.avg_value,
                  'yes_percentage', CASE WHEN q.question_type = 'boolean' THEN ft.avg_value ELSE NULL END,
                  'response_count', ft.response_count
                )
                ORDER BY ft.period_number
              )
              FROM filtered_trends ft
              WHERE ft.question_name = q.question_name
            )
          )
        )
        FROM questions q
        -- Filter to specific question if requested
        WHERE p_question_name IS NULL OR q.question_name = p_question_name
      ),
      'team_size', array_length(v_team_members, 1),
      'campaign_info', (
        SELECT json_build_object(
...
ALTER FUNCTION "public"."get_supervisor_team_trend"("p_campaign_id" "uuid", "p_supervisor_id" "uuid", "p_question_name" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_supervisors_with_min_reportees"("min_reportees" integer DEFAULT 4, "p_campaign_id" "uuid" DEFAULT NULL::"uuid", "p_instance_id" "uuid" DEFAULT NULL::"uuid") RETURNS TABLE("supervisor_id" "uuid", "reportee_count" bigint)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  -- If campaign_id and instance_id are provided, filter by actual responses for that instance
  IF p_campaign_id IS NOT NULL AND p_instance_id IS NOT NULL THEN
    RETURN QUERY
    SELECT 
      us.supervisor_id,
      COUNT(DISTINCT sr.user_id) as reportee_count
    FROM user_supervisors us
    JOIN survey_responses sr ON sr.user_id = us.user_id
    JOIN survey_assignments sa ON sa.id = sr.assignment_id
    WHERE us.is_primary = true
      AND sa.campaign_id = p_campaign_id
      AND sr.campaign_instance_id = p_instance_id
    ...
    HAVING COUNT(DISTINCT sr.user_id) >= min_reportees
    ORDER BY reportee_count DESC;
  ELSE
    -- Original behavior: count all reportees regardless of responses
    RETURN QUERY
    SELECT 
      us.supervisor_id,
      COUNT(us.user_id) as reportee_count
    FROM user_supervisors us
    WHERE us.is_primary = true
    GROUP BY us.supervisor_id
    HAVING COUNT(us.user_id) >= min_reportees
    ORDER BY reportee_count DESC;
  END IF;
END;
$$;
...
ALTER FUNCTION "public"."get_supervisors_with_min_reportees"("min_reportees" integer, "p_campaign_id" "uuid", "p_instance_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_team_data"("p_user_id" "uuid") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_result json;
...
ALTER FUNCTION "public"."get_team_data"("p_user_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_user_active_assignments"("p_user_id" "uuid") RETURNS TABLE("assignment_id" "uuid", "survey_id" "uuid", "survey_name" "text", "campaign_id" "uuid", "campaign_name" "text", "instance_id" "uuid", "instance_period" integer, "starts_at" timestamp with time zone, "ends_at" timestamp with time zone, "status" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
BEGIN
    ...
    SELECT 
        ...
        ci.period_number as instance_period,
        ci.starts_at,
        ci.ends_at,
        ci.status::text
    FROM public.survey_assignments sa
    ...
    LEFT JOIN public.campaign_instances ci ON ci.campaign_id = sc.id AND ci.status = 'active'
    WHERE sa.user_id = p_user_id
        AND (ci.id IS NULL OR ci.ends_at > NOW());
END;
$$;


ALTER FUNCTION "public"."get_user_active_assignments"("p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_user_board_permissions"("p_user_id" "uuid", "p_board_id" "uuid") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_user_board_permissions"("p_user_id" "uuid", "p_board_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."log_login_attempt"("p_user_id" "uuid", "p_email" "text", "p_login_method" "text", "p_success" boolean, "p_error_message" "text" DEFAULT NULL::"text", "p_ip_address" "text" DEFAULT NULL::"text", "p_user_agent" "text" DEFAULT NULL::"text", "p_device_info" "jsonb" DEFAULT '{}'::"jsonb", "p_network_info" "jsonb" DEFAULT '{}'::"jsonb", "p_location_info" "jsonb" DEFAULT '{}'::"jsonb", "p_session_id" "text" DEFAULT NULL::"text") RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
DECLARE
...
ALTER FUNCTION "public"."log_login_attempt"("p_user_id" "uuid", "p_email" "text", "p_login_method" "text", "p_success" boolean, "p_error_message" "text", "p_ip_address" "text", "p_user_agent" "text", "p_device_info" "jsonb", "p_network_info" "jsonb", "p_location_info" "jsonb", "p_session_id" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."populate_response_demographics"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  -- Only populate if the demographic fields are not already set
  -- This prevents unnecessary updates and handles both INSERT and UPDATE cases
  IF NEW.primary_sbu_id IS NULL OR 
     NEW.primary_supervisor_id IS NULL OR 
     NEW.gender IS NULL OR 
     NEW.location_id IS NULL OR 
     NEW.employment_type_id IS NULL OR 
     NEW.employee_type_id IS NULL OR 
     NEW.level_id IS NULL OR 
     NEW.employee_role_id IS NULL OR
     NEW.age IS NULL OR
     NEW.generation IS NULL THEN
    
    -- Get user profile data and populate the demographic fields
    SELECT 
      (SELECT us.sbu_id 
       FROM user_sbus us 
...
ALTER FUNCTION "public"."populate_response_demographics"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."populate_survey_responses_user_data"() RETURNS integer
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
DECLARE
...
ALTER FUNCTION "public"."populate_survey_responses_user_data"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."search_users"("p_search_term" "text" DEFAULT NULL::"text", "p_limit" integer DEFAULT 50, "p_offset" integer DEFAULT 0) RETURNS TABLE("id" "uuid", "email" "text", "first_name" "text", "last_name" "text", "designation" "text", "status" "text", "location_name" "text", "sbu_name" "text", "level_name" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
BEGIN
    ...
    LEFT JOIN public.levels lv ON lv.id = p.level_id
    WHERE (
...
ALTER FUNCTION "public"."search_users"("p_search_term" "text", "p_limit" integer, "p_offset" integer) OWNER TO "postgres";
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'Instance not found';
    RETURN;
  END IF;
...
CREATE OR REPLACE FUNCTION "public"."update_campaign_instance_status"() RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
DECLARE
...
ALTER FUNCTION "public"."update_campaign_instance_status"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."update_last_reminder_sent"("assignment_ids" "uuid"[]) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
BEGIN
...
ALTER FUNCTION "public"."update_last_reminder_sent"("assignment_ids" "uuid"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."update_survey_response_user_data"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
BEGIN
...
ALTER FUNCTION "public"."update_survey_response_user_data"() OWNER TO "postgres";
...
ALTER TABLE "public"."ai_feedback_analysis" OWNER TO "postgres";
...
ALTER VIEW "public"."demographic_gender_analysis" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."issue_board_permissions" (
    ...
    "rule_name" "text",
    "rule_type" "text" DEFAULT 'include'::"text",
    "priority" integer DEFAULT 100,
    "is_active" boolean DEFAULT true,
    CONSTRAINT "issue_board_permissions_rule_type_check" CHECK (("rule_type" = ANY (ARRAY['include'::"text", 'exclude'::"text"])))
);
...
CREATE OR REPLACE VIEW "public"."managers_needing_improvement" WITH ("security_invoker"='on') AS
 ...
 SELECT "manager_id",
    ...
    "total_ratings",
    "average_score",
    "rank"() OVER (ORDER BY "average_score") AS "improvement_rank"
   FROM "manager_scores"
  WHERE ("average_score" > (0)::numeric)
 LIMIT 10;


ALTER VIEW "public"."managers_needing_improvement" OWNER TO "postgres";
...
ALTER VIEW "public"."recent_activities" OWNER TO "postgres";


CREATE OR REPLACE VIEW "public"."response_trends" WITH ("security_invoker"='on') AS
 SELECT "date_trunc"('day'::"text", "created_at") AS "response_date",
    "count"(*) AS "response_count",
...
ALTER VIEW "public"."response_trends" OWNER TO "postgres";
...
CREATE OR REPLACE VIEW "public"."silent_employees" WITH ("security_invoker"='on') AS
 ...
 SELECT "id",
    ...
    "total_assignments",
        CASE
            WHEN ("total_assignments" = 0) THEN 'No Assignments'::"text"
            WHEN ("total_responses" = 0) THEN 'Never Responded'::"text"
            ELSE 'Low Participation'::"text"
        END AS "participation_status"
   FROM "employee_participation"
  WHERE (("total_responses" = 0) OR ((("total_responses")::double precision / (NULLIF("total_assignments", 0))::double precision) < (0.2)::double precision))
  ORDER BY "total_assignments" DESC, "last_response_date"
 LIMIT 10;


ALTER VIEW "public"."silent_employees" OWNER TO "postgres";
...
ALTER VIEW "public"."survey_response_trends" OWNER TO "postgres";
...
ALTER VIEW "public"."top_performing_managers" OWNER TO "postgres";
...
ALTER VIEW "public"."top_performing_sbus" OWNER TO "postgres";
...
ALTER VIEW "public"."upcoming_survey_deadlines" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."ai_feedback_analysis"
    ADD CONSTRAINT "ai_feedback_analysis_campaign_id_instance_id_supervisor_id_key" UNIQUE ("campaign_id", "instance_id", "supervisor_id");



ALTER TABLE ONLY "public"."ai_feedback_analysis"
    ADD CONSTRAINT "ai_feedback_analysis_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."survey_responses"
    ADD CONSTRAINT "survey_responses_assignment_id_user_id_campaign_instance_key" UNIQUE ("assignment_id", "user_id", "campaign_instance_id");
...
CREATE INDEX "idx_ai_feedback_analysis_campaign_instance" ON "public"."ai_feedback_analysis" USING "btree" ("campaign_id", "instance_id");



CREATE INDEX "idx_ai_feedback_analysis_supervisor" ON "public"."ai_feedback_analysis" USING "btree" ("supervisor_id");



CREATE INDEX "idx_issue_board_permissions_active" ON "public"."issue_board_permissions" USING "btree" ("is_active") WHERE ("is_active" = true);
...
CREATE INDEX "idx_issue_board_permissions_board_priority" ON "public"."issue_board_permissions" USING "btree" ("board_id", "priority" DESC, "is_active");
...
CREATE INDEX "idx_survey_responses_assignment_user_instance" ON "public"."survey_responses" USING "btree" ("assignment_id", "user_id", "campaign_instance_id");
...
CREATE OR REPLACE TRIGGER "a_populate_response_demographics_trigger" BEFORE INSERT OR UPDATE ON "public"."survey_responses" FOR EACH ROW EXECUTE FUNCTION "public"."populate_response_demographics"();
...
CREATE OR REPLACE TRIGGER "update_survey_responses_updated_at" AFTER UPDATE ON "public"."survey_responses" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
ALTER TABLE ONLY "public"."ai_feedback_analysis"
    ADD CONSTRAINT "ai_feedback_analysis_campaign_id_fkey" FOREIGN KEY ("campaign_id") REFERENCES "public"."survey_campaigns"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."ai_feedback_analysis"
    ADD CONSTRAINT "ai_feedback_analysis_instance_id_fkey" FOREIGN KEY ("instance_id") REFERENCES "public"."campaign_instances"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."ai_feedback_analysis"
    ADD CONSTRAINT "ai_feedback_analysis_supervisor_id_fkey" FOREIGN KEY ("supervisor_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."survey_responses"
    ADD CONSTRAINT "fk_survey_responses_employee_role" FOREIGN KEY ("employee_role_id") REFERENCES "public"."employee_roles"("id") NOT VALID;



ALTER TABLE ONLY "public"."survey_responses"
    ADD CONSTRAINT "fk_survey_responses_employee_type" FOREIGN KEY ("employee_type_id") REFERENCES "public"."employee_types"("id") NOT VALID;



ALTER TABLE ONLY "public"."survey_responses"
    ADD CONSTRAINT "fk_survey_responses_employment_type" FOREIGN KEY ("employment_type_id") REFERENCES "public"."employment_types"("id") NOT VALID;



ALTER TABLE ONLY "public"."survey_responses"
    ADD CONSTRAINT "fk_survey_responses_level" FOREIGN KEY ("level_id") REFERENCES "public"."levels"("id") NOT VALID;



ALTER TABLE ONLY "public"."survey_responses"
    ADD CONSTRAINT "fk_survey_responses_location" FOREIGN KEY ("location_id") REFERENCES "public"."locations"("id") NOT VALID;



ALTER TABLE ONLY "public"."survey_responses"
    ADD CONSTRAINT "fk_survey_responses_primary_sbu" FOREIGN KEY ("primary_sbu_id") REFERENCES "public"."sbus"("id") NOT VALID;



ALTER TABLE ONLY "public"."survey_responses"
    ADD CONSTRAINT "fk_survey_responses_primary_supervisor" FOREIGN KEY ("primary_supervisor_id") REFERENCES "public"."profiles"("id") NOT VALID;
...
CREATE POLICY "Admins can insert system logs" ON "public"."campaign_instance_status_logs" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."user_roles"
  WHERE (("user_roles"."user_id" = "auth"."uid"()) AND ("user_roles"."role" = 'admin'::"public"."user_role")))));
...
CREATE POLICY "Admins can view all login history" ON "public"."login_history" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."user_roles"
  WHERE (("user_roles"."user_id" = "auth"."uid"()) AND ("user_roles"."role" = 'admin'::"public"."user_role")))));
...
CREATE POLICY "Admins can view system logs" ON "public"."campaign_instance_status_logs" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."user_roles"
  WHERE (("user_roles"."user_id" = "auth"."uid"()) AND ("user_roles"."role" = 'admin'::"public"."user_role")))));
...
CREATE POLICY "Allow anonymous insert for active sessions" ON "public"."live_session_participants" FOR INSERT TO "anon", "authenticated" WITH CHECK ((EXISTS ( SELECT 1
...
CREATE POLICY "Allow reading participants for active sessions" ON "public"."live_session_participants" FOR SELECT TO "anon", "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."live_survey_sessions" "s"
  WHERE (("s"."id" = "live_session_participants"."session_id") AND ("s"."status" = ANY (ARRAY['initial'::"public"."session_status", 'active'::"public"."session_status"]))))));
...
CREATE POLICY "Enable read access for admin" ON "public"."ai_feedback_analysis" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."user_roles"
  WHERE (("user_roles"."user_id" = "auth"."uid"()) AND ("user_roles"."role" = 'admin'::"public"."user_role")))));
...
CREATE POLICY "System processes can insert logs" ON "public"."campaign_instance_status_logs" FOR INSERT WITH CHECK (("auth"."uid"() IS NULL));



CREATE POLICY "Users can create issues with board permissions" ON "public"."issues" FOR INSERT WITH CHECK ((COALESCE((("public"."get_user_board_permissions"("auth"."uid"(), "board_id") ->> 'can_create'::"text"))::boolean, false) OR (EXISTS ( SELECT 1
   FROM "public"."user_roles"
  WHERE (("user_roles"."user_id" = "auth"."uid"()) AND ("user_roles"."role" = 'admin'::"public"."user_role"))))));
...
CREATE POLICY "Users can downvote with board permissions" ON "public"."issue_downvotes" USING (("auth"."uid"() = "user_id")) WITH CHECK ((("auth"."uid"() = "user_id") AND (EXISTS ( SELECT 1
   FROM "public"."issues" "i"
  WHERE (("i"."id" = "issue_downvotes"."issue_id") AND COALESCE((("public"."get_user_board_permissions"("auth"."uid"(), "i"."board_id") ->> 'can_vote'::"text"))::boolean, false))))));



CREATE POLICY "Users can insert their own team feedback analysis" ON "public"."ai_feedback_analysis" FOR INSERT WITH CHECK (("supervisor_id" = "auth"."uid"()));
...
CREATE POLICY "Users can read downvotes with board permissions" ON "public"."issue_downvotes" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."issues" "i"
  WHERE (("i"."id" = "issue_downvotes"."issue_id") AND (COALESCE((("public"."get_user_board_permissions"("auth"."uid"(), "i"."board_id") ->> 'can_view'::"text"))::boolean, false) OR ("auth"."uid"() = "i"."created_by") OR (EXISTS ( SELECT 1
           FROM "public"."user_roles"
          WHERE (("user_roles"."user_id" = "auth"."uid"()) AND ("user_roles"."role" = 'admin'::"public"."user_role")))))))));



CREATE POLICY "Users can read issues with board permissions" ON "public"."issues" FOR SELECT USING ((COALESCE((("public"."get_user_board_permissions"("auth"."uid"(), "board_id") ->> 'can_view'::"text"))::boolean, false) OR ("auth"."uid"() = "created_by") OR (EXISTS ( SELECT 1
   FROM "public"."user_roles"
  WHERE (("user_roles"."user_id" = "auth"."uid"()) AND ("user_roles"."role" = 'admin'::"public"."user_role"))))));



CREATE POLICY "Users can read votes with board permissions" ON "public"."issue_votes" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."issues" "i"
  WHERE (("i"."id" = "issue_votes"."issue_id") AND (COALESCE((("public"."get_user_board_permissions"("auth"."uid"(), "i"."board_id") ->> 'can_view'::"text"))::boolean, false) OR ("auth"."uid"() = "i"."created_by") OR (EXISTS ( SELECT 1
           FROM "public"."user_roles"
          WHERE (("user_roles"."user_id" = "auth"."uid"()) AND ("user_roles"."role" = 'admin'::"public"."user_role")))))))));
...
CREATE POLICY "Users can update their own team feedback analysis" ON "public"."ai_feedback_analysis" FOR UPDATE USING (("supervisor_id" = "auth"."uid"()));
...
CREATE POLICY "Users can view their own team feedback analysis" ON "public"."ai_feedback_analysis" FOR SELECT USING ((("supervisor_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."user_supervisors" "us"
  WHERE (("us"."supervisor_id" = "auth"."uid"()) AND ("us"."user_id" = "us"."supervisor_id"))))));



CREATE POLICY "Users can vote with board permissions" ON "public"."issue_votes" USING (("auth"."uid"() = "user_id")) WITH CHECK ((("auth"."uid"() = "user_id") AND (EXISTS ( SELECT 1
   FROM "public"."issues" "i"
  WHERE (("i"."id" = "issue_votes"."issue_id") AND COALESCE((("public"."get_user_board_permissions"("auth"."uid"(), "i"."board_id") ->> 'can_vote'::"text"))::boolean, false))))));
...
ALTER TABLE "public"."campaign_instance_status_logs" ENABLE ROW LEVEL SECURITY;
...
GRANT ALL ON FUNCTION "public"."activate_campaign_instances"() TO "anon";
GRANT ALL ON FUNCTION "public"."activate_campaign_instances"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."activate_campaign_instances"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."check_user_survey_access"("p_user_id" "uuid", "p_survey_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."check_user_survey_access"("p_user_id" "uuid", "p_survey_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_user_survey_access"("p_user_id" "uuid", "p_survey_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."complete_campaign_instances"() TO "anon";
GRANT ALL ON FUNCTION "public"."complete_campaign_instances"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."complete_campaign_instances"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_dimension_radiogroup"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_question_name" "text", "p_dimension" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_dimension_radiogroup"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_question_name" "text", "p_dimension" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_dimension_radiogroup"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_question_name" "text", "p_dimension" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_supervisor_team_feedback"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_supervisor_id" "uuid", "p_question_name" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_supervisor_team_feedback"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_supervisor_id" "uuid", "p_question_name" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_supervisor_team_feedback"("p_campaign_id" "uuid", "p_instance_id" "uuid", "p_supervisor_id" "uuid", "p_question_name" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_supervisor_team_trend"("p_campaign_id" "uuid", "p_supervisor_id" "uuid", "p_question_name" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_supervisor_team_trend"("p_campaign_id" "uuid", "p_supervisor_id" "uuid", "p_question_name" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_supervisor_team_trend"("p_campaign_id" "uuid", "p_supervisor_id" "uuid", "p_question_name" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_supervisors_with_min_reportees"("min_reportees" integer, "p_campaign_id" "uuid", "p_instance_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_supervisors_with_min_reportees"("min_reportees" integer, "p_campaign_id" "uuid", "p_instance_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_supervisors_with_min_reportees"("min_reportees" integer, "p_campaign_id" "uuid", "p_instance_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_team_data"("p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_team_data"("p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_team_data"("p_user_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_user_active_assignments"("p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_user_active_assignments"("p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_user_active_assignments"("p_user_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_user_board_permissions"("p_user_id" "uuid", "p_board_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_user_board_permissions"("p_user_id" "uuid", "p_board_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_user_board_permissions"("p_user_id" "uuid", "p_board_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."log_login_attempt"("p_user_id" "uuid", "p_email" "text", "p_login_method" "text", "p_success" boolean, "p_error_message" "text", "p_ip_address" "text", "p_user_agent" "text", "p_device_info" "jsonb", "p_network_info" "jsonb", "p_location_info" "jsonb", "p_session_id" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."log_login_attempt"("p_user_id" "uuid", "p_email" "text", "p_login_method" "text", "p_success" boolean, "p_error_message" "text", "p_ip_address" "text", "p_user_agent" "text", "p_device_info" "jsonb", "p_network_info" "jsonb", "p_location_info" "jsonb", "p_session_id" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."log_login_attempt"("p_user_id" "uuid", "p_email" "text", "p_login_method" "text", "p_success" boolean, "p_error_message" "text", "p_ip_address" "text", "p_user_agent" "text", "p_device_info" "jsonb", "p_network_info" "jsonb", "p_location_info" "jsonb", "p_session_id" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."populate_response_demographics"() TO "anon";
GRANT ALL ON FUNCTION "public"."populate_response_demographics"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."populate_response_demographics"() TO "service_role";



GRANT ALL ON FUNCTION "public"."populate_survey_responses_user_data"() TO "anon";
GRANT ALL ON FUNCTION "public"."populate_survey_responses_user_data"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."populate_survey_responses_user_data"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."search_users"("p_search_term" "text", "p_limit" integer, "p_offset" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."search_users"("p_search_term" "text", "p_limit" integer, "p_offset" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_users"("p_search_term" "text", "p_limit" integer, "p_offset" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_campaign_instance_status"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_campaign_instance_status"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_campaign_instance_status"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_last_reminder_sent"("assignment_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."update_last_reminder_sent"("assignment_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_last_reminder_sent"("assignment_ids" "uuid"[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."update_survey_response_user_data"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_survey_response_user_data"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_survey_response_user_data"() TO "service_role";
...
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."achievement_progress" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."achievement_progress" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."achievement_progress" TO "service_role";
...
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."ai_feedback_analysis" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."ai_feedback_analysis" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."ai_feedback_analysis" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."analysis_prompts" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."analysis_prompts" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."analysis_prompts" TO "service_role";
...
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."campaign_instance_status_logs" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."campaign_instance_status_logs" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."campaign_instance_status_logs" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."campaign_instances" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."campaign_instances" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."campaign_instances" TO "service_role";
...
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."demographic_gender_analysis" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."demographic_gender_analysis" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."demographic_gender_analysis" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."email_config" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."email_config" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."email_config" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."employee_roles" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."employee_roles" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."employee_roles" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."employee_types" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."employee_types" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."employee_types" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."employment_types" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."employment_types" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."employment_types" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_board_permissions" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_board_permissions" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_board_permissions" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_boards" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_boards" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_boards" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_downvotes" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_downvotes" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_downvotes" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_votes" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_votes" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."issue_votes" TO "service_role";
...
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."levels" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."levels" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."levels" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_session_participants" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_session_participants" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_session_participants" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_session_questions" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_session_questions" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_session_questions" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_session_responses" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_session_responses" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_session_responses" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_survey_sessions" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_survey_sessions" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."live_survey_sessions" TO "service_role";
...
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."survey_assignments" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."survey_assignments" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."survey_assignments" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."surveys" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."surveys" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."surveys" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."user_supervisors" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."user_supervisors" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."user_supervisors" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."managers_needing_improvement" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."managers_needing_improvement" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."managers_needing_improvement" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."survey_campaigns" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."survey_campaigns" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."survey_campaigns" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."recent_activities" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."recent_activities" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."recent_activities" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."response_trends" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."response_trends" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."response_trends" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."sbus" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."sbus" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."sbus" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."shared_presentations" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."shared_presentations" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."shared_presentations" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."user_sbus" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."user_sbus" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."user_sbus" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."silent_employees" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."silent_employees" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."silent_employees" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."survey_response_trends" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."survey_response_trends" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."survey_response_trends" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."system_versions" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."system_versions" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."system_versions" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."top_performing_managers" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."top_performing_managers" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."top_performing_managers" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."top_performing_sbus" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."top_performing_sbus" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."top_performing_sbus" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."upcoming_survey_deadlines" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."upcoming_survey_deadlines" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."upcoming_survey_deadlines" TO "service_role";
