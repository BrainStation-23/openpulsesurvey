...
async function addComparisonChart(
  ...
  try {
    // Fetch dimension data based on question type
    ...
    if (!dimensionData || dimensionData.length === 0) {
      slide.addText("No comparison data available for this dimension", {
        x: 0.5,
        y: 2,
        w: "90%",
        fontSize: 16,
    ...
    if (questionType === "boolean") {
      // Create a grouped bar chart for boolean comparison
      const labels = dimensionData.map((item: any) => item.dimension);
      const yesValues = dimensionData.map((item: any) => item.yes_count);
      const noValues = dimensionData.map((item: any) => item.no_count);
      ...
      // Create a stacked bar chart for NPS comparison
      const labels = dimensionData.map((item: any) => item.dimension);
      const detractors = dimensionData.map((item: any) => item.detractors);
      const passives = dimensionData.map((item: any) => item.passives);
      const promoters = dimensionData.map((item: any) => item.promoters);
      ...
      // Add a small table with NPS scores by dimension
      slide.addText("NPS Scores by Dimension", {
        x: 0.5,
        y: 6,
        fontSize: 14,
        bold: true,
      ...
      const tableRows = dimensionData.map((item: any) => [
        item.dimension,
      ...
      // Create a stacked bar chart for rating comparison
      const labels = dimensionData.map((item: any) => item.dimension);
      const unsatisfied = dimensionData.map((item: any) => item.unsatisfied);
      const neutral = dimensionData.map((item: any) => item.neutral);
      const satisfied = dimensionData.map((item: any) => item.satisfied);
      
      const chartData = [
        {
          name: "Unsatisfied (1-2)",
          values: unsatisfied,
          labels: labels
        },
        {
      ...
      // Add a small table with average scores by dimension
      slide.addText("Average Scores by Dimension", {
        x: 0.5,
        y: 6,
        fontSize: 14,
        bold: true,
    ...
    console.error(`Error creating comparison chart for ${dimension}:`, error);
    
    // Add error message to slide
    slide.addText(`Comparison data not available for ${dimension}`, {
      x: 0.5,
      y: 2,
      w: "90%",
      fontSize: 16,
...
serve(async (req) => {
  ...
  try {
    // Parse request body
    const { campaignId, instanceId, config, fileName } = await req.json();
    ...
    // Generate the PPTX
    const pptxBuffer = await generatePptx(campaignId, instanceId, config, progressCallback);
    ...
    return new Response(
      pptxBuffer,
      {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="${fileName || `survey_presentation_${campaignId}.pptx`}"`,
          'Cache-Control': 'no-cache'
        }
      }
    );
  } catch (error) {
    console.error("Error in pptx-export function:", error);
